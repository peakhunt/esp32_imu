...
static uint32_t StX=0; // position along the horizonal axis 0 to 20 
static uint32_t StY=0; // position along the vertical axis 0 to 15
static uint16_t StTextColor = ST7735_YELLOW;
static int32_t Ymax,Ymin,X;        // X goes from 0 to 127
static int32_t Yrange; //YrangeDiv2;
...
static uint8_t _width_buffer[256 * 2];
...
{
#if 0 // hkim performance optimize
...
ST7735_FillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
  ...
  for(y=h; y>0; y--)
  {
    for(x=w; x>0; x--)
    {
      _width_buffer[(w-x) * 2 + 0] = hi;
      _width_buffer[(w-x) * 2 + 1] = lo;
    }
    spi_sw_transmit(_width_buffer, w * 2);
  }
  ST7735_CS_HIGH();
#endif
}
...
ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h)
{
  ...
  for(y=0; y<h; y=y+1)
  {
    for(x=0; x<w; x=x+1)
    {
      _width_buffer[x * 2 + 0] = (uint8_t)(image[i] >> 8);
      _width_buffer[x * 2 + 1] = (uint8_t)(image[i]);
      i = i + 1;                        // go to the next pixel
    }
...
// Output: none
void
Output_Color(uint32_t newColor)
{
